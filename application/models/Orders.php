...
class Orders extends CI_Model {
    ...
    {
        $items = [];
        foreach($this->db->select('products')->from('orders')->where('ordered',0)->get()->result_array() as $k=>$v)
        {
    ...
    {
        return $this->db->select('packageID,createdAt,receivedAt')->from('packages')->order_by('receivedAt','ASC')->order_by('createdAt','DESC')->get()->result_array();
    }
    public function getPackageById($id)
    {
        return $this->db->select('*')->from('packages')->where('packageID',$id)->get()->result_array()[0];
    }
    public function listOrdersByIds($id)
    {
        ...
        foreach($id as $key)
        {
            $row = $this->db->select('id,customerID,orderCreated,products,totalPay,totalCost,totalProfit,status')->from('orders')->where('id',$key)->get()->result_array()[0];
    ...
    {
        return $this->db->select('name')->from('customers')->where('id',$id)->get()->result_array()[0]['name'];
    }
